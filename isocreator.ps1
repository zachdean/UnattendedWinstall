# Enable debugging
#Set-PSDebug -Trace 1
# credit the iso making part to https://github.com/ntdevlabs/tiny11builder
...
{
    Write-Host "Restarting unattendwininstall image creator as admin in a new window, you can close this one."
...
Start-Transcript -Path "$ScratchDisk\unattendwininstall.log" 

$Host.UI.RawUI.WindowTitle = "unattendwininstall image creator"
Clear-Host
Write-Host "Welcome to the unattendwininstall image creator! Release: 01-12-25"

$hostArchitecture = $Env:PROCESSOR_ARCHITECTURE
New-Item -ItemType Directory -Force -Path "$ScratchDisk\unattendwininstall\sources" | Out-Null
do {
...
if ((Test-Path "$DriveLetter\sources\boot.wim") -eq $false -or (Test-Path "$DriveLetter\sources\install.wim") -eq $false) {
    if ((Test-Path "$DriveLetter\sources\install.esd") -eq $true) {
        ...
        Export-WindowsImage -SourceImagePath $DriveLetter\sources\install.esd -SourceIndex $index -DestinationImagePath $ScratchDisk\unattendwininstall\sources\install.wim -Compressiontype Maximum -CheckIntegrity
    } else {
...
Copy-Item -Path "$DriveLetter\*" -Destination "$ScratchDisk\unattendwininstall" -Recurse -Force | Out-Null
Set-ItemProperty -Path "$ScratchDisk\unattendwininstall\sources\install.esd" -Name IsReadOnly -Value $false > $null 2>&1
Remove-Item "$ScratchDisk\unattendwininstall\sources\install.esd" > $null 2>&1
Write-Host "Copy complete!"
Start-Sleep -Seconds 2
Clear-Host
Write-Host "Getting image information:"
Get-WindowsImage -ImagePath $ScratchDisk\unattendwininstall\sources\install.wim
$index = Read-Host "Please enter the image index"
Write-Host "Mounting Windows image. This may take a while."
$wimFilePath = "$ScratchDisk\unattendwininstall\sources\install.wim"
...
Mount-WindowsImage -ImagePath $ScratchDisk\unattendwininstall\sources\install.wim -Index $index -Path $ScratchDisk\scratchdir
...
$imageInfo = & 'dism' '/English' '/Get-WimInfo' "/wimFile:$($ScratchDisk)\unattendwininstall\sources\install.wim" "/index:$index"
...
Export-WindowsImage -SourceImagePath $ScratchDisk\unattendwininstall\sources\install.wim -SourceIndex $index -DestinationImagePath $ScratchDisk\unattendwininstall\sources\install2.wim -CompressionType Fast
Remove-Item -Path "$ScratchDisk\unattendwininstall\sources\install.wim" -Force | Out-Null
Rename-Item -Path "$ScratchDisk\unattendwininstall\sources\install2.wim" -NewName "install.wim" | Out-Null
Write-Host "Windows image completed. Continuing with boot.wim."
Start-Sleep -Seconds 2
Clear-Host
Write-Host "Mounting boot image:"
$wimFilePath = "$ScratchDisk\unattendwininstall\sources\boot.wim" 
& takeown "/F" $wimFilePath | Out-Null
& icacls $wimFilePath "/grant" "$($adminGroup.Value):(F)"
Set-ItemProperty -Path $wimFilePath -Name IsReadOnly -Value $false
Mount-WindowsImage -ImagePath $ScratchDisk\unattendwininstall\sources\boot.wim -Index 2 -Path $ScratchDisk\scratchdir
Write-Host "Loading registry..."
...
Dismount-WindowsImage -Path $ScratchDisk\scratchdir -Save
Clear-Host
Write-Host "The unattendwininstall image is now completed. Proceeding with the making of the ISO..."
Write-Host "Copying unattended file for bypassing MS account on OOBE..."
Copy-Item -Path "$PSScriptRoot\autounattend.xml" -Destination "$ScratchDisk\unattendwininstall\autounattend.xml" -Force | Out-Null
...
& "$OSCDIMG" '-m' '-o' '-u2' '-udfver102' "-bootdata:2#p0,e,b$ScratchDisk\unattendwininstall\boot\etfsboot.com#pEF,e,b$ScratchDisk\unattendwininstall\efi\microsoft\boot\efisys.bin" "$ScratchDisk\unattendwininstall" "$PSScriptRoot\unattendwininstall.iso"

# Finishing up
Write-Host "Creation completed! Press any key to exit the script..."
Read-Host "Press Enter to continue"
Write-Host "Performing Cleanup..."
Remove-Item -Path "$ScratchDisk\unattendwininstall" -Recurse -Force | Out-Null
